...
describe('Sistema Integrado de Gestão - Validação Restrição do Edital', () => {
  it.only('CT-ES-06 -  Validação de Restrições - Marcado sem preenchimento da duração', () => {
    ...
    // Resultado esperado: O sistema IMPEDE o usuário de seguir para o próximo substep "Termo de Aceite"
    // Verifica se a nova etapa está visível para o usuário
    cy.contains('Termo de Aceite');

  })
})

// Relatorio de Erros:
// O CT-ES-06 apresenta um erro no sistema, ao clicar na checkbox 'Definir a duração do projeto em meses' e o 
// usuário não preencher o campo de duração do projeto que aparece logo embaixo, o sistema deveria impedir o
// prosseguimento mas não é isso que acontece e o próximo substep "Termo de Aceite" é vísivel.
...
// Frequência: alta, é um fluxo normal o usuário não preencher algo obrigatório
