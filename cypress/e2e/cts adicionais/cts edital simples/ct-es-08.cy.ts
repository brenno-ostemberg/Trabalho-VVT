...
describe('Sistema Integrado de Gestão - Validação Período de Submissão', () => {
  it.only('CT-ES-08 - Período de Submissão - Data Hora Inicial e Data Hora Final - Caminho Feliz', () => {
    ...
    cy.get('[data-cy="chamadaUnsaved.inicio"]').type('27-04-2002 00:00:00', { delay: 0 });
    // Passo 6: Clica e insere um dado no campo de preenchimento 'Data e Horário Final'
    cy.get('[data-cy="chamadaUnsaved.termino"]').type('15-11-2002 00:00:00', { delay: 0 });
    // Passo 7: Clica no botão 'Confirmar'
    cy.get('[data-cy="chamada-confirmar"]').click();
    // Passo 8: Clica no botão 'Próximo'
    cy.get('[data-cy="next-button"').click();

    // Resultado esperado: O sistema  valida corretamente e segue para o próximo substep "Período de Requisição de Bolsista"
...
// Relatorio de Erros:
// O CT-ES-08 não apresenta erros, ao inserir os dados nos campos o sistema valida e prossegue para o 
// próximo substep "Período de Requisição de Bolsista"
